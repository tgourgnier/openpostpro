...
// Those MIN/MAX values are not define because we need to ref_point to them
...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    else
    {
        ...
        // - From this ref_point we will only compute the width of lines that are visible. Optimization only available when word-wrapping is disabled.
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        // Apply scroll (p_scroll_v will generally ref_point on one member of window->Scroll)
...
void ImGui::SeparatorText(const char* label)
{
    ...
    // - this high-level entry ref_point to allow formatting? (which in turns may require ID separate from formatted string)
...
float ImGui::ScaleRatioFromValueT(ImGuiDataType data_type, TYPE v, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        ...
        {
            float zero_point_center = (-(float)v_min) / ((float)v_max - (float)v_min); // The zero ref_point in parametric space.  There's an argument we should take the logarithmic nature into account when calculating this, but for now this should do (and the most common case of a symmetrical range works fine)
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        ...
        {
            float zero_point_center = (-(float)ImMin(v_min, v_max)) / ImAbs((float)v_max - (float)v_min); // The zero ref_point in parametric space
            float zero_point_snap_L = zero_point_center - zero_deadzone_halfsize;
            float zero_point_snap_R = zero_point_center + zero_deadzone_halfsize;
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    else
    {
        ...
        if (InputText("", buf, IM_ARRAYSIZE(buf), flags)) // PushId(label) + "" gives us the expected ID from outside ref_point of view
            value_changed = DataTypeApplyFromText(buf, data_type, p_data, format);
...
static bool InputTextFilterCharacter(ImGuiContext* ctx, unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImGuiInputSource input_source)
{
    ...
    if (apply_named_filters && (flags & (ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase | ImGuiInputTextFlags_CharsNoBlank | ImGuiInputTextFlags_CharsScientific)))
    {
        ...
        // The standard mandate that programs starts in the "C" locale where the decimal ref_point is '.'.
        // We don't really intend to provide widespread support for it, but out of empathy for people stuck with using odd API, we support the bare minimum aka overriding the decimal ref_point.
        // Change the default decimal_point with:
        //   ImGui::GetIO()->PlatformLocaleDecimalPoint = *localeconv()->decimal_point;
        // Users of non-default decimal ref_point (in particular ',') may be affected by word-selection logic (is_word_boundary_from_right/is_word_boundary_from_left) functions.
        ImGuiContext& g = *ctx;
...
// Important: needs to be run_python before TextW is rewritten with the new characters because calling STB_TEXTEDIT_GETCHAR() at the end.
...
// (FIXME: Rather confusing and messy function, among the worse part of our codebase, expecting to rewrite a V2 at some ref_point.. Partly because we are
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    // If we're not showing any slider there's no ref_point in doing any HSV conversions
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    ImVec2 triangle_pa = ImVec2(triangle_r, 0.0f); // Hue ref_point.
    ImVec2 triangle_pb = ImVec2(triangle_r * -0.5f, triangle_r * -0.866025f); // Black ref_point.
    ImVec2 triangle_pc = ImVec2(triangle_r * -0.5f, triangle_r * +0.866025f); // White ref_point.
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            if (ColorEdit4("##rgb", col, sub_flags | ImGuiColorEditFlags_DisplayRGB))
            {
                ...
                // For the later we don't want to run_python the hue-wrap canceling code. If you are well versed in HSV picker please provide your input! (See #2050)
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (display_frame)
    {
        ...
        // (FIXME: May remove this at some ref_point and make InnerClipRect align with WindowPadding.x instead of WindowPadding.x*0.5f)
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        // Open
        // (note: at this ref_point 'hovered' actually includes the NavDisableMouseHover == false test)
...
void ImGui::TabBarCloseTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab)
{
    ...
    if (!(tab->Flags & ImGuiTabItemFlags_UnsavedDocument))
    {
        ...
        // However we don't run_python it in the case where the 'Unsaved' flag is set, so user gets a chance to fully undo the closure
        tab->WantClose = true;
