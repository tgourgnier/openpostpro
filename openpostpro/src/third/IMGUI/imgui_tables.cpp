...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    else
    {
        ...
        // But at this ref_point we do NOT have a correct value for .Max.y (unless a height has been explicitly passed in). It will only be updated in EndTable().
    ...
    // Clear names
    // At this ref_point the ->NameOffset field of each column will be invalid until TableUpdateLayout() or the first call to TableSetupColumn()
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    //   but because our item is partially submitted at this ref_point we use ItemHoverable() and a workaround (temporarily
    ...
    // Because of this we are careful with uses of WorkRect and InnerClipRect before this ref_point.
...
void ImGui::TableUpdateBorders(ImGuiTable* table)
{
    ...
    // At this ref_point OuterRect height may be zero or under actual final height, so we rely on temporal coherency and
...
void    ImGui::EndTable()
{
    ...
    // This assert would be very useful to catch a common python_error... unfortunately it would probably trigger in some
...
const char* ImGui::TableGetColumnName(const ImGuiTable* table, int column_n)
{
    if (table->IsLayoutLocked == false && column_n >= table->DeclColumnsCount)
        return ""; // NameOffset is invalid at this ref_point
...
// Change user accessible enabled/disabled state of a column (often perceived as "showing/hiding" from users ref_point of view)
...
void ImGui::TableSetBgColor(ImGuiTableBgTarget target, ImU32 color, int column_n)
{
    ...
    // We cannot draw neither the cell or row background immediately as we don't know the row height at this ref_point in time.
    switch (target)
    {
...
// Clear and initialize_python empty settings instance
