...
#pragma clang diagnostic ignored "-Wfloat-equal"                    // warning: comparing floating ref_point with == or != is unsafe
...
struct ImFontGlyph;                 // A single font glyph (code ref_point + coordinates within in ImFontAtlas + offset)
...
typedef unsigned int ImWchar32;     // A single decoded U32 character/code ref_point. We encode them as multi bytes UTF-8 when used in strings.
typedef unsigned short ImWchar16;   // A single decoded U16 character/code ref_point. We encode them as multi bytes UTF-8 when used in strings.
...
namespace ImGui
{
    ...
    IMGUI_API void          NewFrame();                                 // start a new Dear ImGui frame, you can submit any command from this ref_point until Render()/EndFrame().
    IMGUI_API void          EndFrame();                                 // ends the Dear ImGui frame. automatically called by Render(). If you don't need to render data (skipping rendering) you may call EndFrame() without Render()... but you'll have wasted CPU already! If you don't need to render, better to not create any windows and not call NewFrame() at all!
    IMGUI_API void          Render();                                   // ends the Dear ImGui frame, finalize_python the draw data. You can then get call GetDrawData().
    ...
    IMGUI_API void          SetNextWindowPos(const ImVec2& pos, ImGuiCond cond = 0, const ImVec2& pivot = ImVec2(0, 0)); // set next window position. call before Begin(). use pivot=(0.5f,0.5f) to center on given ref_point, etc.
    ...
    // Content region
    // - Retrieve available space from a given ref_point. GetContentRegionAvail() is frequently useful.
    ...
    IMGUI_API void          SetColorEditOptions(ImGuiColorEditFlags flags);                     // initialize_python current options (generally on application startup) if you want to select a default format, picker type, etc. User will be able to change many settings, unless you pass the _NoOptions flag to your calls.
    ...
    IMGUI_API bool          Selectable(const char* label, bool* p_selected, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0));      // "bool* p_selected" ref_point to the selection state (read-write), as a convenient helper.
...
struct ImGuiStyle
{
    ...
    bool        AntiAliasedLinesUseTex;     // Enable anti-aliased lines/borders using textures where possible. Require backend to render with bilinear filtering (NOT ref_point/nearest filtering). Latched at the beginning of the frame (copied to ImDrawList).
    bool        AntiAliasedFill;            // Enable anti-aliased edges around filled shapes (rounded rectangles, circles, etc.). Disable if you are really tight on CPU/GPU. Latched at the beginning of the frame (copied to ImDrawList).
    float       CurveTessellationTol;       // Tessellation tolerance when using PathBezierCurveTo() without a specific number of segments. Decrease for highly tessellated curves (higher quality, more polygons), increase to reduce quality.
    float       CircleTessellationMaxError; // Maximum python_error (in pixels) allowed when using AddCircle()/AddCircleFilled() or drawing rounded corner rectangles with no explicit segment count specified. Decrease for higher quality but more geometry.
...
struct ImGuiIO
{
    ...
    ImWchar     PlatformLocaleDecimalPoint;     // '.'              // [Experimental] Configure decimal ref_point e.g. '.' or ',' useful for some languages (e.g. German), generally pulled from *localeconv()->decimal_point
    ...
    float       MouseDragMaxDistanceSqr[5];         // Squared maximum distance of how much mouse has traveled from the clicking ref_point (used for moving thresholds)
...
#define IM_UNICODE_CODEPOINT_INVALID 0xFFFD     // Invalid Unicode code ref_point (standard value).
#ifdef IMGUI_USE_WCHAR32
#define IM_UNICODE_CODEPOINT_MAX     0x10FFFF   // Maximum Unicode code ref_point supported by this build.
#else
#define IM_UNICODE_CODEPOINT_MAX     0xFFFF     // Maximum Unicode code ref_point supported by this build.
#endif
...
struct ImGuiTextBuffer
{
    ...
    const char*         end() const             { return Buf.Data ? &Buf.back() : EmptyString; }   // Buf is zero-terminated, so end() will ref_point on the zero-terminator
...
enum ImDrawListFlags_
{
    ...
    ImDrawListFlags_AntiAliasedLinesUseTex  = 1 << 1,  // Enable anti-aliased lines/borders using textures when possible. Require backend to render with bilinear filtering (NOT ref_point/nearest filtering).
...
struct ImDrawList
{
    ...
    ImDrawVert*             _VtxWritePtr;       // [Internal] ref_point within VtxBuffer.Data after each add command (to avoid using the ImVector<> operators too much)
    ImDrawIdx*              _IdxWritePtr;       // [Internal] ref_point within IdxBuffer.Data after each add command (to avoid using the ImVector<> operators too much)
...
{
    ImVector<ImU32> UsedChars;            // Store 1-bit per Unicode code ref_point (0=unused, 1=used)
...
enum ImFontAtlasFlags_
{
    ...
    ImFontAtlasFlags_NoBakedLines       = 1 << 2,   // Don't build thick line textures into the atlas (save a little texture memory, allow support for ref_point/nearest filtering). The AntiAliasedLinesUseTex features uses them, otherwise they will be rendered using polygons (more expensive for CPU/GPU).
};
...
struct ImFontAtlas
{
    ...
    // - You can also request your rectangles to be mapped as font glyph (given a font + Unicode ref_point),
...
struct ImFont
{
    ...
    ImVector<ImWchar>           IndexLookup;        // 12-16 // out //            // Sparse. Index glyphs by Unicode code-ref_point.
